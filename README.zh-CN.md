...
**Spring Boot Starter，集成 debezium-embedded**
...
> **当前版本**：`2.3.x.20250820-SNAPSHOT`<br>
> **JDK 基线**：`1.8`<br>
...
**debezium-embedded-spring-boot-starter** 是一个面向 使用 debezium-embedded 的应用 的 Spring Boot Starter，用于将 **debezium-embedded** 集成到 Spring Boot 应用中。它提供自动装配、属性绑定与开箱即用的 Bean，使应用以最小配置即可使用 debezium-embedded 的全部能力。
...
| JDK | `1.8` |
| 坐标 | `io.github.easy4j:debezium-embedded-spring-boot-starter:2.3.x.20250820-SNAPSHOT` |
...
| `RowEventHandler` | classpath + property | 不创建 |
...
| 属性 | 类型 | 默认值 | 必填 | 说明 | 敏感 |
|---|---|---|:---:|---|:---:|
| `debezium.embedded.enabled` | boolean | `true` | 否 | 是否启用 Starter | 否 |
<!-- 更多属性见下方 -->
...
| 症状 | 诊断 | 解决 |
|---|---|---|
| Bean 未创建 | 查看自动装配报告 | 确认 `debezium.embedded.enabled=true` 与 classpath |
...
[返回顶部](#readme-top) · [问题反馈](https://github.com/easy-4-java/debezium-embedded-spring-boot-starter/issues) · [仓库地址](https://github.com/easy-4-java/debezium-embedded-spring-boot-starter)
